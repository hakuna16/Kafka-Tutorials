...
public class LibraryEventProducer {
    ...
    public void sendLibraryEvent(final LibraryEvent libraryEvent) throws JsonProcessingException {
        ...
        ListenableFuture<SendResult<Integer, String>> futureValue = kafkaTemplate.sendDefault(key, value);

        futureValue.addCallback(new ListenableFutureCallback<SendResult<Integer, String>>() {
            @Override
    ...
    /**
     * Here we are using {@link KafkaTemplate} to send the messages to the topic.
     * We can have explicitly provided the topic and can be sent to n number of topic.
     * using {@link KafkaTemplate#send(String, Object)}.
     */
    public void sendLibraryEventUsingSend(final LibraryEvent libraryEvent) throws JsonProcessingException {

        final Integer key = libraryEvent.getLibraryEventId();
        final String value = objectMapper.writeValueAsString(libraryEvent);

        ListenableFuture<SendResult<Integer, String>> futureValue = kafkaTemplate
     ...
     * Here we are using {@link KafkaTemplate} to send the messages to the topic.
     * We can have explicitly provided the topic and can be sent to n number of topic.
     * using {@link org.apache.kafka.clients.producer.ProducerRecord}
     */
    public void sendLibraryEventUsingSendWithProducerRecord(final LibraryEvent libraryEvent) throws JsonProcessingException {
    ...
    private void handleSuccess(final Integer key, final String value, final SendResult<Integer, String> result) {
        log.info("Message sent for the key: {} and the value is:{} and the partition is:{}",
